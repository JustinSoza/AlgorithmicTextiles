...
function BedNeedle(bed, needle) {
	if (arguments.length == 1 && typeof(arguments[0]) === 'string') {
		...
		if (!m) {
			throw new Error(`ERROR: invalid needle specification '${str}'.`);
		}
		this.bed = m[1];
		...
		throw new Error("Don't know how to construct a BedNeedle from the given arguments");
	}
}
...
function Pass(info) {
	...
	['type', 'slots', 'direction', 'carriers', 'hook', 'gripper', 'racking', 'pause', 'stitch', 'speed', 'subRoller', 'presserMode'].forEach(function(name){
	...
	console.assert('subRoller' in this, "Can't specify a pass without a sub roller value.");
	...
	} else if (this.type === TYPE_A_MISS) {
		console.assert(!('gripper' in this), "Must use gripper only on KNIT_TUCK pass.");
		console.assert(!('hook' in this), "Must use hook only on KNIT_TUCK pass.");
		console.assert(this.carriers.length === 0, "A-miss passes cannot have carriers specified.");
	} else {
...
Pass.prototype.hasFront = function() {
	console.assert(this.type === TYPE_KNIT_TUCK || this.type == TYPE_A_MISS, "It only makes sense to ask knit-tuck [or amiss] passes if they have front stitches.");
...
Pass.prototype.hasBack = function() {
	console.assert(this.type === TYPE_KNIT_TUCK || this.type == TYPE_A_MISS, "It only makes sense to ask knit-tuck [or amiss] passes if they have back stitches.");
	for (let s in this.slots) {
...
Pass.prototype.append = function(pass) {
	...
	if (!['type', 'racking', 'stitch', 'speed', 'subRoller', 'direction', 'carriers'].every(function(name){
...
(function knitoutToPasses() {
	...
	(function checkVersion(){
		...
		if (!m) {
			throw "File starts with '" + lines[lineIdx] + "', which is not a valid knitout magic string";
		}
		if (parseInt(m[1]) > 2) {
	...
	let subRoller = 0; //machine-specific subRoller
	...
	for ( ; lineIdx < lines.length; ++lineIdx) {
		...
		if (op === 'in' || op === 'inhook') {
		...
		} else if (op == 'x-sub-roller-number') {
			throwIfSVS();
			if (args.length !== 1) throw "ERROR: x-sub-roller-number takes one argument.";
			if (!/^[+]?\d+$/.test(args[0])) throw "ERROR: x-sub-roller-number argument must be non-negative integer.";
			let newSubRollerNumber = parseInt(args.shift());
			subRoller = newSubRollerNumber;
			...
			if (n.isBack() && t.isFront()) {
				if (n.needle + racking !== t.needle) {
					throw new Error(`ERROR: needles '${n}' and '${t}' are not aligned at racking ${racking}.`);
				}
				...
				if (n.needle !== t.needle + racking) {
					...
					throw new Error(`ERROR: needles '${n}' and '${t}' are not on opposite beds! (Must transfer back <-> front!)`);
			}
...
(function passesToRaster() {
	...
	function carriersToInt(pass){
		if (pass.carriers.length === 0) {
			return ((pass.type === TYPE_KNIT_TUCK || pass.type === TYPE_A_MISS) ? 255 : 0); //use 255 as a way of indicating "yes we really meant no carriers"
			...
			if (pass.carriers[0] === "10") {
				return parseInt("10" + pass.carriers[1]);
			...
			console.warn("Don't know how to notate yarn carrier combination " + JSON.stringify(pass.carriers) + ".");
		}
	}
	...
	passes.forEach(function(pass) {
		...
		if (pass.speed > 15 || pass.speed < 0) {
			console.warn("speed value specified is outside of the valid range 0-15");
		}

		//sub roller: (L10 knits, L11 for transfers. 1-31 are sub rollers 1-31.)
		//we only set L10 as to avoid pulling down the fabric during xfers
		options.L10 = pass.subRoller;

		if (pass.subRoller > 31 || pass.subRoller < 0) {
			console.warn("sub-roller value specified is outside of the valid range 0-31");
		}
		...
		if (pass.type === TYPE_A_MISS) {
			options.L12 = 30; //a miss 2 (front/back)
		}
